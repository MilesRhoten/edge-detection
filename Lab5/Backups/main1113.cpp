...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    */
    ...
    Mat topLeft = frame(Rect(0, 0, midCol, midRow));

    Mat topRight = frame(Rect(midCol, 0, \
			      frame.cols - midCol, midRow));

    Mat botLeft = frame(Rect(0, midRow, \
			     midCol, frame.rows - midRow));

    Mat botRight = frame(Rect(midCol, midRow, \
			      frame.cols - midCol, \
			      frame.rows - midRow));
...
void frametosobel(Mat &input, int ID, Mat &output) {
  ...
  int xOffset = (ID % 2) * sobel.cols;
  int yOffset = (ID / 2) * sobel.rows;


  cv::Rect outRect(xOffset, yOffset, sobel.cols, sobel.rows);


  Rect outSobelRect(0, 0, sobel.cols, sobel.rows);
...
cv::Mat to442_grayscale(cv::Mat &input) {
  ...
  for (y = 0; y < input.rows; y++) {
	...
	(.2126 * pixelPtr[offset + 2]) + (.7152 * pixelPtr[offset + 1]) +
